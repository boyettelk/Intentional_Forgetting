...
def create_false_objects(attr_dict, rule_json, num_objects):
    '''
    Ensures that the generated image will not contain the rule attribute, 
    but be completely randomized for each image 
    Args:
        ...
        rule_json: A json file with an dictionary object for each class_id with the attribute rule that we want to avoid
    Returns:
        false_dict: A completely randomized dictionary without the false rule attributes
    '''
    # read json file
    with open(rule_json) as json_file:
        data = json.load(json_file)
        ...
        for class_id in class_ids:
            ...
            keys = []
            values = []
            #check which attributes are in rule dict
            for key in attr_dict1.keys():                        #iterate through each key
                if rule_dict[key] in list(attr_dict1[key]):      #for the rule attribute:
                    value = rule_dict[key]                       #remove from LIST of possible attributes
                    keys.append(key)
            ...
            objects = []
            for i in range(random.randint(1, num_objects)):
                false_dict = attr_dict.copy()                       #create a new dict
                count = 0
                for key in attr_dict.keys():                        #iterate through keys
                    v = random.choice(list(attr_dict[key]))         #randomly choose a value for each key
                    false_dict[key] = v
                    if v in values:
                        count += 1
                print('false dict before check', false_dict)

                if count >= 2:
                    j = random.choice(range(len(keys)))
                    key_choice = keys[j]
                    value_choice = values[j]
                    list_of_all_values = attr_dict1[key_choice]
                    print('list of all values:', list_of_all_values)
                    list_of_all_values.remove(value_choice)
                    new_value = random.choice(list(attr_dict1[key_choice]))
                    print('new value:', new_value)
                    false_dict[key_choice] = new_value
                    print('new false dict', false_dict)
                    count = 0
            ...
            #also fill in missing values for second object in each class:
            for key in cube_dict.keys():
                if cube_dict[key] == '':
                    v = random.choice(list(attr_dict[key]))
                    cube_dict[key] = v
            objects.append(cube_dict)
